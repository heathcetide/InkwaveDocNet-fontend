...
export const TabsList = ({ className, vertical = false, fullWidth = false, ...props }) => (
    <TabsPrimitive.List
        className={clsx(
            "relative flex gap-1",
            vertical ? "flex-col" : "flex-row items-center justify-center",
            fullWidth ? "w-full" : "",
...
export const TabsTrigger = ({ className, vertical = false, ...props }) => (
...
export const TabsContent = ({ className, ...props }) => (
    <TabsPrimitive.Content
        className={clsx(
            "mt-4 animate-fade-in data-[state=inactive]:hidden",
            className
        )}
        {...props}
    />
